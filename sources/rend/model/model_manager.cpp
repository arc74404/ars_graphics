...
ModelManager::loadTexture(const tinygltf::Image& image,
...
bool
ModelManager::loadTextures(const std::vector<tinygltf::Texture>& gltf_textures,
                           const tinygltf::Model& gltf_model,
...
bool
ModelManager::fileOpenSuccess(const std::string& path) const
{
...
ModelManager::load(const std::string& path)
{
    ...
    for (auto&& gltf_node : gltf_model.nodes)
    {
        ...
        node_data.mesh_index = gltf_node.mesh;
        node_data.matrix =
            getFinalMatrix(gltf_node.rotation, gltf_node.translation,
                           gltf_node.scale, gltf_node.matrix);

        res_model.addNode(std::move(node_data));
